...
from sandbox import ParallelContainer, TaskAllocationSandBox
...
def evaluation():
    ...
    exp.start_experiment('../configs/task_allocation.json')

    env = TaskAllocationSandBox(exp.args['sandbox'], render_mode=None)
    ...
    with tqdm.tqdm(total=max_step) as pbar:
        for c in range(max_step):
            ...
            next_ob, rew, dones, truncation, info = env.step(env.sample())
            # reward_sum += rew
