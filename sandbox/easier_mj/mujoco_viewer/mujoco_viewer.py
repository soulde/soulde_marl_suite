...
class MujocoViewer(Callbacks):
    ...
    def _add_marker_to_scene(self, marker):
        ...
        for key, value in marker.items():
            if isinstance(value, (int, float, mujoco._enums.mjtGeom)):
                ...
                raise ValueError("mjtGeom doesn't.xml have field %s" % key)
    ...
    def _create_overlay(self):
        ...
        if self._image_idx > 0:
            ...
            add_overlay(topleft, "Cap[t.xml]ure frame", "Saved as %s" % fname)
        else:
            add_overlay(topleft, "Cap[t.xml]ure frame", "")
    ...
    def read_pixels(self, camid=None, depth=False, seg=False):
        if self.render_mode == 'window':
            raise NotImplementedError(
                "Use 'render()' in 'window' mode.")
        if seg:
            self.scn.flags[_enums.mjtRndFlag.mjRND_SEGMENT] = True
            self.scn.flags[_enums.mjtRndFlag.mjRND_IDCOLOR] = True
        ...
        if depth:
            ...
            # Scale to [0, 255]
            depth_img = (255 * np.clip(depth_img, 0, 1)).astype(np.uint8)
        else:
        ...
        if seg:
            geom_ids = rgb_img[:, :, 0]
            geom_id2 = rgb_img[:, :, 1]
            # tmp = cv2.resize(geom_ids, [500,500]) / geom_ids.max()
            # print(tmp)
            # Infinity is mapped to -1
            geom_ids = geom_ids.astype(np.float64) + 1
            # Scale to [0, 1]
            geom_ids = geom_ids / geom_ids.max()
            rgb_img = (255 * geom_ids).astype(np.uint8)
